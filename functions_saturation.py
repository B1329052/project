# ============================================================
# 物品盤點程式 - 函式區（飽和度增加 100% 版本）
# ============================================================
# 與 functions.py 的差異：
#   compress_images() 改用 ImageEnhance.Color 增加飽和度 100%
#   不使用對比度調整（ImageEnhance.Contrast）
# ============================================================
...
def compress_images(image_files, output_folder="compressed_images"):
    """
    將圖片清單中的每張圖片縮小尺寸並增加飽和度後存到指定資料夾
    - 使用 thumbnail() 縮小，保持原始比例
    - 使用 ImageEnhance.Color 增加 100% 飽和度
    - 保留原始檔案格式與副檔名
    回傳：處理後圖片路徑的列表
    """
    # 如果輸出資料夾不存在，自動建立
